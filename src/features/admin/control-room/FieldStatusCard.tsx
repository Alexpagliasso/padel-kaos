  ...
  return (
    <article className="rounded border border-white/10 bg-[#171717] p-4">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--event-primary)]">{court?.name ?? 'Campo da definire'}</p>
      ...
      <div className="grid gap-2">
        <ScoreLine name={teamA?.name ?? 'Squadra A'} games={match.score.games.A} />
        <ScoreLine name={teamB?.name ?? 'Squadra B'} games={match.score.games.B} />
      </div>

      <div className="mt-4 grid grid-cols-2 gap-2 text-sm">
        <StatusCell label="Set" value={String(match.score.currentSet)} />
        <StatusCell label="Remaining" value="Timer not available" />
        <StatusCell label="Carta attiva" value={activeCardDefinition?.name ?? 'Nessuna'} />
        <StatusCell label="Kaos State" value={diceRule?.title ?? 'Nessuna'} />
      </div>
    </article>
  )
}
...
function ScoreLine({ name, games }: { name: string; games: number }) {
  return (
    <div className="flex items-center justify-between rounded bg-black/45 px-3 py-2">
      ...
      <span className="text-2xl font-black text-[var(--event-primary)]">{games}</span>
    </div>
  )
}
