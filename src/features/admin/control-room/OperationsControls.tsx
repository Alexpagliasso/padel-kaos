...
import { roundCardsLocked } from '../../../domain/live/cardAssignment'
...
export function OperationsControls({ entry, tournament, matches, roundId, roundName, section = 'all' }: { entry: WorkspaceEntry; tournament: Tournament; matches: Match[]; roundId?: string; roundName?: string; section?: 'all' | 'turn' | 'live' | 'settings' }) {
  ...
  const run = async (operation: () => Promise<unknown>, success: string) => {
    setLiveFeedback('')
    try { await operation(); setLiveFeedback(success); return true } catch (cause) { setLiveFeedback(cause instanceof Error ? cause.message : 'Operazione non riuscita.'); return false }
  }
  ...
  return <Stack spacing={3}>
    ...
    {(section==='all'||section==='turn')&&<Paper sx={{ p: 3 }}>
      ...
      {mode === 'centralized' ? <Stack spacing={1.25} sx={{ mt: 3 }}>
        {tournament.cardsEnabled!==false&&!cardsReady&&<Alert severity="warning" action={<Button color="inherit" disabled={!roundId||live.isPending||cardsLocked} onClick={()=>drawCards(cardsAssigned)}>ASSEGNA CARTE</Button>}>ASSEGNA LE CARTE PRIMA DI AVVIARE IL TURNO · {cardReadyTeams} / {cardTotalTeams} squadre pronte</Alert>}
    ...
    {(section==='all'||section==='live')&&<Paper sx={{ p: 3, background: 'var(--event-gradient)' }}>
      ...
      {diceFace ? <Box sx={{mb:2,p:2,border:1,borderColor:'primary.main',borderRadius:2,textAlign:'center'}}><Typography variant="overline">DADO GLOBALE</Typography><Typography variant="h2">{diceFace.title}</Typography><Typography>{diceFace.shortDescription}</Typography></Box> : <Typography sx={{ mb: 2 }}>{dice ? `Dado globale attuale: ${dice.value}` : 'Dado globale / Non lanciato'}</Typography>}
