...
export function CourtDisplayRoute() {
  ...
  if (!court || !match) {
    return (
      <RoleShell>
        <main className="mx-auto grid min-h-[calc(100svh-66px)] max-w-7xl content-center px-6 py-8">
          <section className="rounded border border-white/10 bg-[#171717] p-8 text-center">
            <p className="text-lg font-black uppercase tracking-[0.2em] text-[#FFD000]">Court Display</p>
            <h1 className="mt-3 text-4xl font-black">No matches configured</h1>
          </section>
        </main>
      </RoleShell>
    )
  ...
  return (
    <RoleShell>
      <main className="mx-auto grid min-h-[calc(100svh-66px)] max-w-7xl content-center gap-6 px-6 py-8">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-lg font-black uppercase tracking-[0.2em] text-[#FFD000]">{court.name}</p>
          ...
          <label className="grid gap-1 text-sm font-bold text-white/55">
            Court
            <select className="rounded border border-white/10 bg-black px-3 py-3 text-white" value={court.id} onChange={(event) => matchRepo.selectCourt(event.target.value)}>
              {tournament.courts.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
            </select>
          </label>
        </div>
        {isKaosPending ? (
          <section className="rounded border border-[#FFD000] bg-[#FFD000] p-10 text-center text-black">
            <p className="text-xl font-black uppercase tracking-[0.22em]">KAOS TIME</p>
            <h2 className="mt-3 text-7xl font-black">WAITING FOR DICE</h2>
          </section>
        ) : (
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          {activeCardDefinition ? (
            <div className="rounded border border-[#FFD000]/30 bg-[#FFD000]/10 p-4">
              <p className="text-sm font-black uppercase text-[#FFD000]">Active Card</p>
