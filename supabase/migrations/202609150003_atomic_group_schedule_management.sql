-- Atomic persistence for the pre-start group-stage calendar.
-- Independent from the deferred recovery migration 202609150002.
begin;
...
begin
  ...
  if exists (
    select 1 from public.groups g
    where g.tournament_id = p_tournament_id and (
      select count(*) from public.profiles p
      where p.tournament_id = p_tournament_id
        and p.role = 'referee'::public.app_role
        and p.court_id = g.assigned_court_id
    ) <> 1
